...
public class ArticleController {
	...
	ResponseEntity<ArticleDTO> createArticle(@RequestBody @Valid final ArticleModificationRequestDTO articleDataDTO) {
		...
		return new ResponseEntity<>(ArticleDTOMapper.toArticleDTO(this.articleService.createArticle(ArticleDTOMapper.fromArticleDTO(articleDataDTO))), HttpStatus.CREATED);
	}
	...
	ResponseEntity<ArticleDTO> updateArticle(@PathVariable final String articleCode, @RequestBody @Valid final ArticleModificationRequestDTO articleDataDTO) {
		LOG.info("Llamada a la API de articulos para modificar la informacion del articulo con codigo: {}", articleCode);
		return new ResponseEntity<>(ArticleDTOMapper.toArticleDTO(this.articleService.updateArticle(articleCode, ArticleDTOMapper.fromArticleDTO(articleDataDTO))), HttpStatus.OK);
	}
	...
	ResponseEntity<ArticleDTO> updatePriceArticle(@PathVariable final String articleCode, @RequestBody @Valid final ArticlePriceModificationRequestDTO articleDataDTO) {
		...
		return new ResponseEntity<>(ArticleDTOMapper.toArticleDTO(this.articleService.updatePriceArticleByCode(articleCode, articleDataDTO.getPrice())), HttpStatus.OK);
	}
